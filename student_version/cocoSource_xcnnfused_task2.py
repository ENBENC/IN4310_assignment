...
class RNN(nn.Module):
        ...
        for i in range(num_rnn_layers):
            if i == 0:
                ...
                if self.cell_type == 'GRU':
                    layer_lst.append(GRUCell(hidden_state_size=self.hidden_state_size, input_size=self.input_size))
                else:
                    layer_lst.append(RNNsimpleCell(hidden_state_size=self.hidden_state_size, input_size=self.input_size))
            else:
                input_size_list.append(self.hidden_state_size)
                if self.cell_type == 'GRU':
                    layer_lst.append(GRUCell(hidden_state_size=self.hidden_state_size, input_size=self.hidden_state_size))
                else:
                    layer_lst.append(RNNsimpleCell(hidden_state_size=self.hidden_state_size, input_size=self.hidden_state_size))
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        for i in range(sequence_length):
            ...
            output_from_prev_layer = None

            for j in range(num_rnn_layers):
                if j == 0:
                    output_from_prev_layer = self.cells[j](input_for_the_first_layer, current_hidden_state[j,:])
                    current_hidden_state[j] = output_from_prev_layer

                else:
                    output_from_prev_layer = self.cells[j](output_from_prev_layer, current_hidden_state[j,:])
                    current_hidden_state[j] = output_from_prev_layer

            #choose the first element in a batch, and make prediction from it
            #after for-loop, the output_from_prev_layer will be the output from the last layer
            logits_i = output_layer(output_from_prev_layer[0, :])
            logits_sequence.append(logits_i)
            predictions = torch.argmax(logits_i, dim=1)
...
class GRUCell(nn.Module):
    def __init__(self, hidden_state_size: int, input_size: int):
        ...
        self.weight_u = nn.Parameter(torch.randn(self.hidden_state_size + input_size, self.hidden_state_size) / np.sqrt(input_size + self.hidden_state_size))
        self.bias_u = nn.Parameter(torch.zeros((1, self.hidden_state_sizes)))
        # Reset gate parameters
        self.weight_r = nn.Parameter(torch.randn(self.hidden_state_size + input_size, self.hidden_state_size) / np.sqrt(input_size + self.hidden_state_size))
        self.bias_r = nn.Parameter(torch.zeros((1, self.hidden_state_sizes)))
        # Hidden state parameters
        self.weight = nn.Parameter(torch.randn(self.hidden_state_size + input_size, self.hidden_state_size) / np.sqrt(input_size + self.hidden_state_size))
    ...
    def forward(self, x, hidden_state):
        ...
        x_mark = torch.cat((x, hidden_state), dim=1) #(batch, hidden+input)
        updata_gate = torch.sigmoid(torch.mm(x_mark, self.weight_u) + self.bias_u) #(batch, hidden)
        reset_gate = torch.sigmoid(torch.mm(x_mark, self.weight_r) + self.bias_r)  #(batch, hidden)

        x_hat = torch.cat((x, (reset_gate * hidden_state)), dim=1) #(batch, hidden+input)

        new_hidden_state_hat = torch.tanh(torch.mm(x_hat, self.weight) + self.bias) #(batch, hidden)
        new_hidden_state = updata_gate * hidden_state + (1-updata_gate) * new_hidden_state_hat #(batch, hidden)
